    ...
    auto stepKinetics(ChemicalField& field) -> ReactiveTransportResult
    {
        ...
        if(options.use_smart_kinetic_solver)
        {
            ...
            for(Index icell = 0; icell < num_cells; ++icell)
            {
                ...
                // Update the amounts of elements for kinetic solver generated by transport
                smart_kinetic_solver.setElementsAmountsPerCell(field[icell], b.row(icell));

                // Solve with a smart kinetic solver
                smart_kinetic_solver.solve(field[icell], t_start, dt);

                std::cout << "n : " << tr(field[icell].speciesAmounts()) << std::endl << std::endl;
                //getchar();
                // Save the result of this cell's smart equilibrium calculation.
                ...
                std::cout << " accepted                : " << result.smart_kinetics_at_cell[icell].estimate.accepted << std::endl;
                ...
                std::cout << "     - equilibration       : " << result.smart_kinetics_at_cell[icell].timing.learn_equilibrate << " (" << result.smart_kinetics_at_cell[icell].timing.learn_equilibrate / result.smart_kinetics_at_cell[icell].timing.solve * 100 << " %)" << std::endl;
        ...
        else
        {
            ...
            for(Index icell = 0; icell < num_cells; ++icell)
            {
                ...
                // Update the amounts of elements for kinetic solver generated by transport
                kinetic_solver.setElementsAmountsPerCell(field[icell], b.row(icell));

                // Solve with a conventional kinetic solver
                kinetic_solver.solve(field[icell], t_start, dt);

                //std::cout << "n : " << tr(field[icell].speciesAmounts()) << std::endl;
                //getchar();
        ...
        auto sum = [&](const ReactiveTransportResult& res){
            for(unsigned int i=0; i < num_cells; ++i)
            {
                if(options.use_smart_kinetic_solver){
                    ...
                    initialize += res.smart_kinetics_at_cell.at(i).timing.learn_initialize;
                    integrate += res.smart_kinetics_at_cell.at(i).timing.learn_integrate;
                    ...
                    equilibration += res.smart_kinetics_at_cell.at(i).timing.learn_equilibrate;
        ...
        std::cout << "number of learnings : " << num_cells - sum_learnings(result) << " out of " << num_cells << std::endl;
        std::cout << "------------------------------------------------" << std::endl;
        getchar();
