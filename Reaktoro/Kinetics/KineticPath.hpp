...
public:
    ...
    /// Solve the kinetic path problem (adaptively in the interval `[t0, t1]` with CVODE).
    ...
    /// Solve the kinetic path problem (uniformly with the `n` time steps of teh size `dt` starting from the time `t0`,
    /// each time step `[t0, t0 + dt]` is solved with internal CVODE-solver adaptively).
    /// @param state The initial state of the chemical system
    /// @param t0 The initial time of the kinetic path
    /// @param t1 The final time of the kinetic path
    /// @param units The time units of `t0` and `t1` (e.g., `s`, `minute`, `day`, `year`, etc.).
