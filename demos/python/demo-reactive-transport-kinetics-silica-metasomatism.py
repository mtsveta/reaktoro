...
# sys.path.insert(1, '/home/skyas/polybox/reaktoro/build-python/lib/python3.7/site-packages')
# #input()
...
nsteps = 20            # the number of steps in the reactive transport simulation
...
T = 300.0 + 273.15  # temperature (in units of Kelvin)
...
os.system('mkdir -p ' + 'plots-' + folder_results)

def titlestr(t):
    t = t / minute  # Convert from seconds to minutes
    h = int(t) / 60  # The number of hours
    m = int(t) % 60  # The number of remaining minutes
    return "time %3dh%2dm" % (h, m)
...
def plot_figures_ph_mpl(steps):
    for i in steps:
        ...
        s.plot(kind='line', x='x', y='pH', label='pH', color='teal', ax=ax)
        plt.legend(loc='lower right', prop=prop)
        #plt.savefig('plots-' + folder_results + '/ph-{}.pdf'.format(i))
        plt.savefig('plots-' + folder_results + '/ph-{}.png'.format(i))
        plt.close()

def plot_figures_ph_simultaneously_mpl(steps):
    ...
    plt.xlabel('Distance [m]', fontproperties=prop)
    plt.ylabel('pH', fontproperties=prop)
    for i, j in zip(steps, list(range(0, len(steps)))):
        t = i * dt
        s = df[df['step'] == i]
        label = str('pH on ' + titlestr(t))
        s.plot(kind='line', x='x', y='pH', color=palette[j], label=label, ax=ax)
    plt.legend(loc='upper right', prop=prop)
    plt.savefig('plots-' + folder_results + '/ph-over-time.pdf'.format(i))
    plt.close()
...
def plot_figures_chrysotile_mpl(steps):
    for i in steps:
        print("On chrysotile figure at time step: {}".format(i))
        t = i * dt
        s = df[df['step'] == i]
        ax = plt.gca()
        #ax.set_ylim(0.0, 0.1)
        ax.set_xlim(xcells[0], xcells[-2])
        plt.xlabel('Distance [m]', fontproperties=prop)
        plt.ylabel('Concentration [mol/m3]', fontproperties=prop)
        plt.title(titlestr(t), fontproperties=prop,fontsize=20)
        s.plot(kind='line', x='x', y='Chrysotile', label='Chrysotile', color='red', ax=ax)
        plt.legend(loc='upper right', prop=prop)
        #plt.savefig('plots-' + folder_results + '/brucite-{}.pdf'.format(i))
        plt.savefig('plots-' + folder_results + '/chrysotile-{}.png'.format(i))
        plt.close()

def plot_figures_talc_mpl(steps):
    for i in steps:
        print("On talc figure at time step: {}".format(i))
        t = i * dt
        s = df[df['step'] == i]
        ax = plt.gca()
        #ax.set_ylim(0.0, 0.1)
        ...
        plt.ylabel('Concentration [mol/m3]', fontproperties=prop)
        plt.title(titlestr(t), fontproperties=prop,fontsize=20)
        s.plot(kind='line', x='x', y='Talc', label='Talc', color='green', ax=ax)
        plt.legend(loc='upper right', prop=prop)
        #plt.savefig('plots-' + folder_results + '/brucite-{}.pdf'.format(i))
        plt.savefig('plots-' + folder_results + '/talc-{}.png'.format(i))
        plt.close()

def plot_figures_brucite_mpl(steps):
    for i in steps:
        print("On brucite figure at time step: {}".format(i))
        t = i * dt
        s = df[df['step'] == i]
        ax = plt.gca()
        #ax.set_ylim(0.0, 0.1)
        ...
        plt.ylabel('Concentration [mol/m3]', fontproperties=prop)
        plt.title(titlestr(t), fontproperties=prop,fontsize=20)
        s.plot(kind='line', x='x', y='Brucite', label='Brucite', color='orange', ax=ax)
        plt.legend(loc='upper right', prop=prop)
        #plt.savefig('plots-' + folder_results + '/brucite-{}.pdf'.format(i))
        plt.savefig('plots-' + folder_results + '/brucite-{}.png'.format(i))
        plt.close()

def plot_figures_forsterite_mpl(steps):
    for i in steps:
        print("On forsterite figure at time step: {}".format(i))
        t = i * dt
        s = df[df['step'] == i]
        ax = plt.gca()
        ax.set_ylim(0.0, 0.1)
        ...
        plt.ylabel('Concentration [mol/m3]', fontproperties=prop)
        plt.title(titlestr(t), fontproperties=prop,fontsize=20)
        s.plot(kind='line', x='x', y='Forsterite', label='Forsterite', color='blue', ax=ax)
        #s.plot(kind='line', x='x', y='hematite', label='Hematite', color='orange', ax=ax)
        #s.plot(kind='line', x='x', y='siderite', label='Siderite', color='green', ax=ax)
        plt.legend(loc='upper right', prop=prop)
        #plt.savefig('plots-' + folder_results + '/pyrite-forsterite-{}.pdf'.format(i))
        plt.savefig('plots-' + folder_results + '/forsterite-{}.png'.format(i))
        plt.close()
...
def plot_figures_aqueous_species_mpl(steps):
    ...
    for i in steps:
        ...
        plt.yscale('log')
        plt.title(titlestr(t), fontproperties=prop,fontsize=20)
        s.plot(kind='line', x='x', y='Hcation', label=r'H$^{+}$', color='darkcyan', ax=ax)
        s.plot(kind='line', x='x', y='Mgcation', label=r'Mg$^{2+}$', color='darkorange', ax=ax)
        s.plot(kind='line', x='x', y='SiO2aq', label=r'SiO$_2$(aq)', color='gray', ax=ax)
        plt.legend(loc='lower right', prop=prop)
        #plt.savefig('plots-' + folder_results + '/aqueous-species-{}.pdf'.format(i))
        plt.savefig('plots-' + folder_results + '/aqueous-species-{}.png'.format(i))
        plt.close()

def plot_figures_fe_simultaneously_mpl(steps):

    palette_fe = sns.color_palette("Blues")
    ...
    for i, j in zip(steps, list(range(0, len(steps)))):
        t = i * dt
        s = df[df['step'] == i]
        s.plot(kind='line', x='x', y='Fe2cation', label=r'Fe$^{2+}$', color=palette_fe[len(palette_fe)-1-j], ax=ax)
    plt.legend(loc='center right', prop=prop)
    plt.savefig('plots-' + folder_results + '/fe-over-time.pdf'.format(i))
    plt.close()
...
bar = IncrementalBar('RT:', max = nsteps)
...
selected_steps_to_plot = [1, 5, 10, 15, 20]
...
plot_figures_forsterite_mpl(selected_steps_to_plot)
plot_figures_brucite_mpl(selected_steps_to_plot)
plot_figures_chrysotile_mpl(selected_steps_to_plot)
plot_figures_talc_mpl(selected_steps_to_plot)
