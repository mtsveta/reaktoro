...
struct ReactiveTransportSolver::Impl
{
    ...
    */
    ...
    : system(system), equilibrium_solver(system), smart_equilibrium_solver(system), kinetic_solver(reactions)
    {
        setBoundaryState(ChemicalState(system));
    ...
    auto stepKinetics(ChemicalField& field) -> ReactiveTransportResult
    {
        ...
        else
        {
            ...
            for(Index icell = 0; icell < num_cells; ++icell)
            {
                ...
                std::cout << "   - equilibration       : " << result.kinetics_at_cell[icell].timing.equilibration << " (" << result.kinetics_at_cell[icell].timing.equilibration / result.kinetics_at_cell[icell].timing.solve * 100 << " %)" << std::endl;
                */
                /*
